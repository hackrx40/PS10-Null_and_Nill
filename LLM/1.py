...
if PERSIST and os.path.exists("persist"):
    vectorstore = Chroma(persist.directory ="persist" ,embedding_function = OpenAIEmbeddings())

    index = VectorStoreIndexWrapper(vectorstore = vectorstore)
else:
    loader = DirectoryLoader("")
    if PERSIST:
        index = VectorstoreIndexCreator(vectorstore_kwargs = {"persist_directory"}).from_loader([loader])
    else:
